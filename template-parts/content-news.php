...
<div class="post-listings clear">
	...
	if ($latest_post_id) {
		...
		$excerpt = ($content) ? shortenText($content,300) : '';
		...
		<?php if ($paged<2) { ?>
			...
			<div class="lastest_post_image clear">
				<?php if ($img) { ?>
				<img class="featImg" src="<?php echo $img[0] ?>" alt="<?php echo $latest_post_title;?>" />		
				<?php } else { ?>
	...
	<div class="post-items-wrap clear">
		...
		<?php while ( $theposts->have_posts() ) : $theposts->the_post();  ?>
			...
			$pexcerpt = ($pcontent) ? shortenText($pcontent,100) : '';
			?>
			<a class="post-item-block" href="<?php echo get_permalink(); ?>">
				<?php if ($pImg) { ?>
					<span class="image clear <?php echo ($pImg) ? 'has-image': 'no-image';?>"><img class="feat" src="<?php echo $pImageSrc ?>" alt="<?php echo get_the_title() ?>" /></span>
				<?php } ?>
