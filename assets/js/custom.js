  ...
  *
  ...
  window.onload = function () {
    ...
    if ($('#video').length) {
      ...
      var playPromise = document.getElementById('video'); // if (playPromise !== undefined) {
      //     playPromise.then(_ => {
      ...
      var playButton = document.getElementById("play-pause"); // Event listener for the play/pause button

      playButton.addEventListener("click", function () {
        if (video.paused == true) {
          // Play the video
          video.play(); // Update the button text to 'Pause'
          ...
          // Pause the video
          video.pause(); // Update the button text to 'Play'
      ...
      if (target.length) {
        $('html, body').animate({
          ...
          if ($target.is(":focus")) {
            ...
            $target.attr('tabindex', '-1'); //$target.focus(); // Set focus again
          }
